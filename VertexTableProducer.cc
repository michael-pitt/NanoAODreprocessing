...
VertexTableProducer::VertexTableProducer(const edm::ParameterSet& params)
    : pvs_(consumes<std::vector<reco::Vertex>>(params.getParameter<edm::InputTag>("pvSrc"))),
      ...
      pc_(consumes<pat::PackedCandidateCollection>(params.getParameter<edm::InputTag>("packedPFCandidates1"))),
...
void VertexTableProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  auto otherPVsTable =
      std::make_unique<nanoaod::FlatTable>((*pvsIn).size() > 5 ? 5 : (*pvsIn).size(), "Other" + pvName_, false);
  std::vector<float> pvsz;
  std::vector<int> pvsntracks0p5;
  std::vector<int> pvsntracks0p9;
  int ntr0p5=0;
  int ntr0p9=0;
  for (size_t i = 0; i < (*pvsIn).size() && i < 5; i++){
     ...
     for (auto iter_pf = pc_handle->begin(); iter_pf != pc_handle->end(); iter_pf++) {
        ...
        const pat::PackedCandidate::PVAssoc pvassoc=pc.fromPV(i);
        bool passChargeSel0p5(pc.pt()>0.5); 
 	bool passChargeSel0p9(pc.pt()>0.9 && fabs(pc.eta())<2.1); 
        if(passChargeSel0p5 && pvassoc>pat::PackedCandidate::PVTight){
           ntr0p5++;
        }
        if(passChargeSel0p9 && pvassoc>pat::PackedCandidate::PVTight){
           ntr0p9++;
        }
     }
     pvsntracks0p5.push_back(ntr0p5);
     pvsntracks0p9.push_back(ntr0p9);
  }
  otherPVsTable->addColumn<float>(
      "z", pvsz, "Z position of other primary vertices", nanoaod::FlatTable::FloatColumn, 8);
  otherPVsTable->addColumn<int>(
      "ntracks0p9", pvsntracks0p9, "Number of tracks of primary vertices pt>0.9", nanoaod::FlatTable::IntColumn);
  otherPVsTable->addColumn<int>(
      "ntracks0p5", pvsntracks0p5, "Number of tracks of primary vertices pt>0.5", nanoaod::FlatTable::IntColumn);
